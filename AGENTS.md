...
- The persisted store is at **version 2**. Any new required `UserProfile` field needs a `migrate` branch, and the engine should still tolerate its absence (`profile.riskTolerance ?? 3`).
